...
function populatePanelForDate(ymd) {
  ...
  const dateLabel = new Date(+y, +mo - 1, +d).toLocaleString('default', {
    day: 'numeric', month: 'long', year: 'numeric'
  });
  document.getElementById('panelDate').textContent = dateLabel;

  const dayEvents = events.filter(ev => ev.parsedDate === ymd && isPublic(ev));
...
function showFullMonthEvents() {
  ...
  const monthEvents = events.filter(ev => ev.parsedDate.startsWith(monthStr) && isPublic(ev));
  renderEventList(monthEvents, true);
}
